    ...
    public class SQLiteHelper
    {
        ...
        public void Insert(string tableName, Dictionary<string, object> dic)
        {
            ...
            foreach (KeyValuePair<string, object> kv in dic)
            {
                ...
                if (!kv.Key.ToString().ToLower().StartsWith("Id"))
                {
                    sbCol.Append("`");
                    sbCol.Append(kv.Key);
                    sbCol.Append("`");
        ...
        public void Update(string tableName, Dictionary<string, object> dicData, string colCond, object varCond)
        {
            ...
            dicCond[colCond] = varCond;
            Update(tableName, dicData, dicCond);
        }
        ...
        public int LastInsertRowId(string ID_Name, string Table_Name)
        {
            long LastId = ExecuteScalar<long>("select last_insert_rowid();");
            return ExecuteScalar<int>("SELECT " + ID_Name + " FROM " + Table_Name + " WHERE _rowid_=" + LastId.ToString());
        }        
